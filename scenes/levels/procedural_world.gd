...
func generate_chunk(starting_coords: Vector2):
	...
	chunk_cache[str(starting_coords)] = {
		...
		'plant_1_tiles': plant_1_tiles,
		'flower_tiles': flower_tiles,
	}
	# set the tiles using the tile arrays
	tileMap.render_chunks(chunk_cache[str(starting_coords)]);
	all_land_tiles.append_array(land_tiles);

func get_chunks_in_range():
	var chunk_count = sqrt(chunks);
	if terrain_set:
		chunk_count *= 2;
	for x in chunk_count:
		for y in chunk_count:
			var x_pos_change = (x * width + grid_position.x) - (chunk_count / 2) * width;
			var y_pos_change = (y * height + grid_position.y) - (chunk_count / 2) * height;
